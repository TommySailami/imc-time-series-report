...
Each series is the full 100-ms-tick mid-price for the named product
across the three Round 4 days — roughly 30 000 observations apiece. The
raw IMC order-book dumps were pre-processed by `scripts/clean_round4.R`
into two-column (`t`, `mid_price`) CSVs; see Appendix A.
...
<img src="Round4_Report_files/figure-gfm/fig1-1.png" alt="Figure 1. Hydrogel mid-price over three days. Dashed line — sample mean."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig2-1.png" alt="Figure 2. Velvetfruit mid-price."  />
<p class="caption">

Figure 2. Velvetfruit mid-price.
</p>
...
<img src="Round4_Report_files/figure-gfm/fig3a-1.png" alt="Figure 3a. Sample ACF of hydrogel."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig3b-1.png" alt="Figure 3b. Sample PACF of hydrogel."  />
<p class="caption">
...
**Interpretation.** A sharp lag-1 spike in either the ACF or PACF
followed by little structure is the bid-ask-bounce signature for
tick-level price data with a tight spread. Whether the picture leans
AR-like or MA-like is decided below by EACF and BIC.
...
<img src="Round4_Report_files/figure-gfm/fig4-1.png" alt="Figure 4. Subset-ARMA BIC plot. The y-axis is sorted from lowest BIC at the top; black squares show which lags enter the model on each row."  />
<p class="caption">
...
m3 = arima(train, order = c(0, 0, 1))     # MA(1)        — bid-ask bounce
...
<img src="Round4_Report_files/figure-gfm/fig5-1.png" alt="Figure 5. tsdiag() page for the BIC-best model."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig6-1.png" alt="Figure 6. Q-Q plot of residuals."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig7a-1.png" alt="Figure 7a. plot.Arima() forecast for the BIC-best hydrogel model with 95 percent prediction limits."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig7b-1.png" alt="Figure 7b. Same forecast page for the BIC runner-up. Compare to Figure 7a — if the two forecasts and bands are nearly identical, the model choice does not matter much for prediction."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig8-1.png" alt="Figure 8. Box–Cox profile log-likelihood. Vertical references: lambda = 0 (log, dashed) and lambda = 1 (identity, dotted)."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig9a-1.png" alt="Figure 9a. Velvetfruit on the chosen modelling scale."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig9b-1.png" alt="Figure 9b. First difference of the modelling-scale series."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig10a-1.png" alt="Figure 10a. ACF of differenced velvetfruit."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig10b-1.png" alt="Figure 10b. PACF of differenced velvetfruit."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/vf-armasubsets-1.png" alt="Figure 11. BIC subset plot for the differenced velvetfruit series. The top row is the BIC-best subset ARMA on the differences — equivalently the BIC-best ARIMA(p, 1, q) on the level."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig12-1.png" alt="Figure 12. tsdiag() page for the BIC-best velvetfruit model."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig13-1.png" alt="Figure 13. Q-Q plot of residuals."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig14a-1.png" alt="Figure 14a. plot.Arima() forecast for the BIC-best velvetfruit model on the price scale (back-transformed via exp() if log was used)."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig14b-1.png" alt="Figure 14b. Same forecast page for the BIC runner-up. Compare against Figure 14a."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig15-1.png" alt="Figure 15. Twenty Monte Carlo paths from the fitted velvetfruit model (thin lines) overlaid on the actual training series (thick line)."  />
<p class="caption">
...
<img src="Round4_Report_files/figure-gfm/fig16-1.png" alt="Figure 16. P&amp;L histogram for a toy mean-reversion rule applied to 1000 simulated paths. Vertical line — P&amp;L on the actual training data."  />
<p class="caption">
...
1)-vs-AR(1) ambiguity by parsimony. A negative MA coefficient (when
MA(1) wins) is the quantitative signature of bid-ask bounce. Forecasts
...
The analysis reads two cleaned CSVs in `data/`, produced by
`scripts/clean_round4.R` from the IMC Round 4 raw order-book dumps:

| File | Columns | Rows |
|:---|:---|:---|
| `hydrogel_mid.csv` | `t`, `mid_price` | one per 100-ms tick across days 1-3 (~30 000) |
| `velvetfruit_mid.csv` | `t`, `mid_price` | one per 100-ms tick across days 1-3 (~30 000) |

`t` is a global tick index (1, 2, 3, …) running continuously across the
three days. `mid_price` is $(bid_1 + ask_1) / 2$ from the original IMC
`prices_round_4_day_{d}.csv` order-book snapshots, filtered to a single
product. The cleaning script discards everything else (bid/ask depth,
volumes, the per-algorithm `profit_and_loss` column, and the
`VEV_{strike}` voucher rows). Trades data (`trades_round_4_day_{d}.csv`)
...
1.  Ensure `data/hydrogel_mid.csv` and `data/velvetfruit_mid.csv` are
    present. They are produced by `scripts/clean_round4.R`, which reads
    the raw IMC `prices_round_4_day_{1, 2, 3}.csv` files. If the cleaned
    CSVs are already committed to the repo, this step can be skipped.
2.  Open `Round4_Report.Rmd` in RStudio.
3.  Click **Knit** (or run `rmarkdown::render("Round4_Report.Rmd")` from
    the console).

The first knit will install `TSA` from CRAN if it is not already
present.

> **Runtime note.** With ~30 000 observations per series,
> `armasubsets()`, `BoxCox.ar()`, and the 1 000-path Monte Carlo block
> each take a noticeable moment. Total knit time on a modern laptop is
> roughly one to two minutes. While drafting, dropping the MC path count
> from 1 000 to 200 in the `fig16` chunk speeds iteration considerably.
