...
``` r
acf(train, lag.max = 10, main = "ACF — hydrogel")
```
...
``` r
pacf(train, lag.max = 10, main = "PACF — hydrogel")
```
...
``` r
acf(diff(y), lag.max = 10, main = "ACF — diff(y)")
```
...
``` r
pacf(diff(y), lag.max = 10, main = "PACF — diff(y)")
```
...
**Interpretation.** Read the table for a triangle of `o`’s whose
upper-left vertex gives the candidate (p, q). Here the (0, 0) cell is
`o` but (0, 2) is already `x`, so the corner is broken — there is no
clean low-order vertex. The first cell with all-`o`’s to the right and
below sits up around (3, 5), which is implausibly complex for a price
difference and almost certainly chance. The absence of a clean low-order
corner is itself consistent with “differences are white noise” — EACF
does not single out an ARMA(p, q) alternative to ARIMA(0, 1, 0).
...
differences — equivalently, the BIC-best ARIMA(p, 1, q) on the level.
Here the BIC-best row has only `error-lag6` shaded (BIC ≈ −0.79),
i.e. an isolated θ₆ MA correction. The intercept-only baseline sits at
BIC ≈ 0, so the improvement from adding that one term is under 1 BIC
unit — well below the “decisive” threshold of 2 — and the chosen lag (6)
has no economic story behind it. Like the ACF/PACF and EACF, the subset
plot fails to identify a meaningful low-order ARMA structure on the
differences, leaving the random walk ARIMA(0, 1, 0) as the parsimonious
default; the AIC/BIC table next confirms this numerically.
...
**Interpretation.** ARIMA(0, 1, 0) wins decisively: BIC = 1935.19 versus
1940.74 for both ARIMA(0, 1, 1) and ARIMA(1, 1, 0), a gap of 5.55 units
(above 4 is decisive). Even the lighter-penalty AIC also picks the
random walk. Adding either an MA(1) or AR(1) correction improves the
in-sample log-likelihood only marginally — not enough to offset the BIC
complexity penalty for the extra parameter. We therefore conclude that
the velvetfruit level follows a pure random walk on this training
window.
...
**Interpretation.** The thick line is the actual velvetfruit training
series; the 20 thin lines are synthetic paths generated by the fitted
ARIMA(0, 1, 0), each starting at y₁ and adding independent random shocks
of variance σ² over the same 270 ticks. The real series sits inside the
cloud, indicating that the data is a plausible draw from the model’s
distribution. The synthetic paths fan out further than the real series
does — most reach the 5050–5400 range by tick 270 while the realised
data stays near 5200–5300 — which suggests the actual data is slightly
less volatile than the fitted σ would imply, though not enough to call
